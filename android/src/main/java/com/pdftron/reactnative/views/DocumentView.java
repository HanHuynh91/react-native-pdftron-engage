...
public class DocumentView extends com.pdftron.pdf.controls.DocumentView {
    ...
    public void setPageLabel(ReadableArray mapping) throws PDFNetException {
        PDFViewCtrl pdfViewCtrl = getPdfViewCtrl();
        PDFDoc pdfDoc = pdfViewCtrl.getDoc();
        ArrayList<Object> item = mapping.toArrayList();
        for (int i = 0; i < item.size(); i++){
            PageLabel L = PageLabel.create(pdfDoc, PageLabel.e_none, item.get(i).toString(), 1);
            pdfDoc.setPageLabel(i+1, L);
        }
    }
