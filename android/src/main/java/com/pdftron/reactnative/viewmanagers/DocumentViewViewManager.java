...
public class DocumentViewViewManager extends ViewGroupManager<DocumentView> {
    ...
    @ReactProp(name = "colorMode")
    public void setColorder(DocumentView documentView, Integer colorMode) {
        documentView.setColorMode(colorMode);
    }
    ...
    public void pageLabel(int tag, ReadableArray item) throws PDFNetException {
        DocumentView documentView = mDocumentViews.get(tag);
        if (documentView != null) {
            documentView.setPageLabel(item);
        } else {
